...
#ifdef SUPPORT_64BIT_OFFSET
        _DLLEXPORT_ void linuxice_format64(const _CodeInfo* ci, const _DInst* di, _DecodedInst* result, PFN_GET_SYMBOL_CALLBACK get_symbol_callback)
#else
        _DLLEXPORT_ void linuxice_format32(const _CodeInfo* ci, const _DInst* di, _DecodedInst* result, PFN_GET_SYMBOL_CALLBACK get_symbol_callback)
#endif
{
	_WString* str;
	...
	for (i = 0; ((i < OPERANDS_NO) && (di->ops[i].type != O_NONE)); i++) {
		...
		switch (di->ops[i].type)
		{
			...
			case O_PC:
			  ...
			  get_symbol_callback((di->imm.sqword + di->addr + di->size) & addrMask, str->p);
			  str->length = strlen(str->p);			  
#else
